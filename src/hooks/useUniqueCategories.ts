...
export const useUniqueCategories: UseUniqueCategoriesHook = (
    ...
    useEffect(() => {
        const uniqueCategories = genres?.flatMap(({ categories }) => categories).filter(filterById) as Category[];

        setUniqueCategories(uniqueCategories);
    }, [genres]);

    useEffect(() => {
        categoriesToKeepRef.current = categories?.filter(
            (category) => uniqueCategories?.find((c) => c?.id === category.id)
        );
